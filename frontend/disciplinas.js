document.addEventListener('DOMContentLoaded', () => {
    ...
    async function buscarDisciplinas() {
        try {
            ...
            disciplinas.forEach(disciplina => {
                const curso = Array.from(seletorCurso.options)
                        .find(opt => Number(opt.value) === disciplina.curso_id);
                const linha = document.createElement('tr');
                linha.innerHTML = `
                    <td>${disciplina.nome}</td>
                    <td>${disciplina.carga_horaria}h</td>
                    <td>${curso ? curso.textContent : '???'}</td>
    ...
    async function prepararEdicao(id) {
        try {
            const resposta = await fetch(apiUrlDisciplinas); // busca todas
