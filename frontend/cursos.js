document.addEventListener('DOMContentLoaded', () => {
    ...
    async function buscarCursos() {
        try {
            ...
            cursos.forEach(curso => {
                const universidade = Array.from(seletorUniversidade.options)
                                     .find(opt => Number(opt.value) === curso.universidade_id);
    ...
    async function prepararEdicao(id) {
        try {
            const resposta = await fetch(apiUrlCursos); // Back-end precisa ter GET /api/cursos/<id>
            if (!resposta.ok) throw new Error('Falha ao buscar dados do curso.');
            
            const cursos = await resposta.json();
            const curso = cursos.find(c => c.id == id); // filtra pelo id
